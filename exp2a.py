...
def new_model(top_layers, dropout):
  ...
  for l in top_layers:
    y = Dense(l, activation='relu')(y)
    y = Dropout(dropout)(y)
  food_category = Dense(16, activation='softmax', name='food_category')(y)
...
def train(model_path=None, num_epoch=DEFAULT_NUM_EPOCH, checkpoint_path=None, learning_rate = 0.01, momentum=0.9, augment=False, weights_path=None):
  ...
  model.compile(loss={'ingre_category': cos_distance,
