...
def train(model_path=None, num_epoch=DEFAULT_NUM_EPOCH, checkpoint_path=None, learning_rate = 0.01, momentum=0.9):
  ...
  if checkpoint_path is not None:
    checkpointer = ModelCheckpoint(checkpoint_path, monitor="acc", mode="max", save_best_only=True, save_weights_only=True)
...
def batch_generator(db, fsq, batch_size=100, partition='train'):
  ...
  while(True):
    ...
    yield (batch_x, {'ingre_category': ingredients, 'food_category': batch_y})
